...
export default class ConfigServic extends SharedConfigService {
    ...
    */
    public getFSClient(): S3ClientConfig {
        return ({
            region: 'us-east-1',
            endpoint: `http://${this.getVal('PRODUCT_SERVICE_S3_HOST')}:${this.getVal('PRODUCT_SERVICE_S3_PORT')}`,
            credentials: {
                accessKeyId: this.getVal('PRODUCT_SERVICE_S3_USER'),
                secretAccessKey: this.getVal('PRODUCT_SERVICE_S3_PASSWORD'),
            },
            forcePathStyle: true,
        })
    }
